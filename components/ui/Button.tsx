  ...
  return (
    <Link href={href} className={`${base} ${shell[variant]} ${className}`}>
      ...
      {variant === 'secondary' && (
        <span
          aria-hidden
          className="absolute inset-0 origin-left scale-x-0 bg-gold transition-transform duration-300 ease-out group-hover/btn:scale-x-100"
        />
      )}
      {/* Brilho que varre a superfície (primary). */}
      {variant === 'primary' && (
        <span
          aria-hidden
